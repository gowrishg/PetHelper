...
# symbols which are imported by "from pethelper.command import *"
...
from pethelper.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("pethelper"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="pethelper.config")
...
def bootstrap():
    ...
    By default this runs the function 'pethelper.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-pethelper = pethelper.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="PetHelper %s" % version)
    ...
    _read_config(args)
    from pethelper.model import bootstrap_model
